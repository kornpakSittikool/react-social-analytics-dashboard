...
export default function GitHubSection({ username }: GitHubSectionProps) {
  ...
  return (
    <section className="mt-8 space-y-8">
      {error && (
        <div className="rounded-2xl border border-red-400/45 bg-red-500/12 p-4 text-sm text-red-100">
          Failed to load GitHub data: {error}
        </div>
      )}

      <div className="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
        <article className="fade-up fade-up-delay-1 rounded-3xl border border-white/10 bg-[#0d1421]/75 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-6">
          {loading && !profile ? (
            ...
            <div>
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-1 inline-block text-sm font-medium text-cyan-300 transition-colors hover:text-cyan-200"
                  >
                    @{profile.login}
                  </a>
              ...
              <dl className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  ...
                  <dd className="mt-1 text-xl font-black text-white">{formatCount(stats.totalRepos)}</dd>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <dt className="text-[11px] uppercase tracking-[0.18em] text-zinc-400">Stars</dt>
                  <dd className="mt-1 text-xl font-black text-white">{formatCount(stats.totalStars)}</dd>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <dt className="text-[11px] uppercase tracking-[0.18em] text-zinc-400">Forks</dt>
                  <dd className="mt-1 text-xl font-black text-white">{formatCount(stats.totalForks)}</dd>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
        ...
        <article className="fade-up fade-up-delay-2 rounded-3xl border border-white/10 bg-[#0d1421]/75 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-lg font-bold tracking-tight text-white">Contribution Activity</h3>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#080b12] p-2 sm:p-4">
            <div className="mx-auto w-full max-w-[930px] overflow-x-auto">
              <img
                ...
                className="mx-auto block h-auto min-w-[760px] sm:min-w-0 sm:w-full"
                loading="lazy"
              />
            </div>
      ...
      <section className="fade-up fade-up-delay-3">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <h3 className="text-xl font-black tracking-tight text-white">Top Repositories</h3>
        ...
        {loading && repos.length === 0 ? (
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0d1421]/70 p-6 text-zinc-300">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {topRepos.map((repo, index) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="fade-up group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0d1421]/75 p-5 shadow-[0_16px_35px_rgba(0,0,0,0.28)] transition-all duration-300 hover:-translate-y-1 hover:border-cyan-300/35 hover:bg-[#111b2d]"
              ...
              >
                ...
                <div className="font-bold tracking-tight text-zinc-50 transition-colors group-hover:text-cyan-200">
                  {repo.name}
                </div>
